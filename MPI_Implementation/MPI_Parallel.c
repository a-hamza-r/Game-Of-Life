...
for (int generations = 0; generations < iterations; ++generations)
{
	...
	{	
		...
		{	//adding a hollow ring up and below
			...
			lohi[0]=((i-1)*iter);
			lohi[1]=end+1;
			MPI_Ssend(&lohi,2,MPI_INT,pRank,0,MPI_COMM_WORLD);
			MPI_Ssend(&currentGen[lohi[0]*(hcols)],(lohi[1]-lohi[0]+1)*(hcols),MPI_C_BOOL,pRank,0,MPI_COMM_WORLD);
			pRank++;
		}
			// printf("HERE\n");
		...
		if (rank <= rows)
		{
			...
			// PrintArray(&newState[0],size-2,hcols);
			// MAIN ALGORITHM END
